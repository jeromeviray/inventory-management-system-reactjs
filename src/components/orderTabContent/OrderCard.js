...
import {
    CCard,
    CCardBody,
    CRow, CCol, CImage, CFormLabel, CFormTextarea
} from '@coreui/react'
...
export class OrderCard extends Component {
    ...
    render() {
        let { orderItem, canReview, rating, comment, orderId, submitted, canViewReview } = this.state;
                                ...
                                {(canReview || (canViewReview && isSubmitted)) &&
                                    <div>
                                        <ReactStars
                                            ...
                                            isHalf={false}
                                            edit={!submitted}
                                            onChange={(newRating) => { this.props.handleOrderReview(orderId, orderItem, newRating, comment) }}
                                        />
                                        <div className="mb-!2">
                                            <textarea
                                                disabled={submitted}
                                                onChange={(event) => { this.props.handleOrderReview(orderId, orderItem, rating, event.target.value) }}
                                                id="exampleFormControlTextarea1" rows="3" style={{ width: "100%" }} placeholder="Please write a review.">{comment}</textarea>
                                        </div>
                                    </div>
                                }
                            </div>
